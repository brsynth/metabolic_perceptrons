...
# Aim: Predict the 2D cluster for Figure 2E
  ...
  for (hip_weight in c(0.03)) {
    ...
    for (max_cluster in c(20)) {
      for (min_cluster in c(2)) {
        ...
        y_low = runif(n_simulations, min = 1 - 0.01  * max_cluster, max = 1)
        ...
        name= paste("2D_cluster_10_11_HipO", weights["hippurate_weight"], "CocE",weights["cocaine_weight"],  '_max_', (100-max_cluster), '_min_', min_cluster, sep = '_')
