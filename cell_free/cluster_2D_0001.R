...
# Aim: Predict the 2D cluster for Figure 2A
...
for (coc_weight in c(0.3, 1)) {
  for (hip_weight in c(0.03)) {
  # for (hip_weight in c(0, 0.1,0.3, 1, 3,10)) {
    ...
    for (max_cluster in c(20)) {
      for (min_cluster in c(2)) {
        ...
        y_high = runif(n_simulations, min = 0, max = 0.01  * min_cluster)
        ...
        for (threshold in c(2.5, 3, 3.5)) {
          ...
          result_threshold = data.frame(line_from_boundary_vertical(boundary_threshold))
        ...
        p <- p + labs(x = "Cocaine", y = "Hippurate", title = "", subtitle = "")
        p <- p + theme_bw() #theme black and white
        print(p)
        
        name= paste("2D_cluster_00_01_HipO", weights["hippurate_weight"], "CocE",weights["cocaine_weight"],  '_max_', (100-max_cluster), '_min_', min_cluster, sep = '_')
