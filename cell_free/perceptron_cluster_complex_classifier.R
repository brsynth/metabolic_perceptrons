...
# Aim: Predict the full complex classifier with clustering of inputs- imperfect signal
...
for (max_cluster in c(20)) {
  for (min_cluster in c(2)) {
    ...
    expected_behavior = all_classifiers_t[which(all_classifiers_t[,c("input")] == "complex"),]
    
    new_results_cluster = data.frame(all_classifiers_t[which(all_classifiers_t[,c("input")] != "or"),])
    ...
    name= paste("cluster_complex_gate_hipo_", weights["hippurate_weight"], '_max_', (100-max_cluster), '_min_', min_cluster, sep = '_')
