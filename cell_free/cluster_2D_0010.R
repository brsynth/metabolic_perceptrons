...
# Aim: Predict the 2D cluster for Figure 2B
  ...
  # for (hip_weight in c(0, 0.1,0.3, 1, 3,10)) {
    ...
    for (max_cluster in c(20)) {
      for (min_cluster in c(2)) {
        ...
        x_high = runif(n_simulations, min = 0, max = 0.01  * min_cluster)
        ...
        name= paste("2D_cluster_00_10_HipO", weights["hippurate_weight"], "CocE",weights["cocaine_weight"],  '_max_', (100-max_cluster), '_min_', min_cluster, sep = '_')
