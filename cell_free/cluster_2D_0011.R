...
# Aim: Predict the 2D cluster for Figure 2C
  ...
  for (hip_weight in c(0.3, 1)) {
    ...
    for (max_cluster in c(20)) {
      for (min_cluster in c(2)) {
        ...
        name= paste("2D_cluster_00_11_HipO", weights["hippurate_weight"], "CocE",weights["cocaine_weight"],  '_max_', (100-max_cluster), '_min_', min_cluster, sep = '_')
