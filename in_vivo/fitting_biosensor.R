...
# Aim: generate and visualise individual fittings on the model on the biosensor
...
# Adress of the helper functions file - contains functions for calculations and visualisation.
...
folder_for_images <- paste(current_folder, "biosensor_single_full_run_1", sep = '/')
...
for (i in 1:run_number) {
  ...
  simulated_data =data.frame(cbind(benzoic_acid_range, calculate_biosensor(parameters = pars_optimised, concentrations = benzoic_acid_range)))
